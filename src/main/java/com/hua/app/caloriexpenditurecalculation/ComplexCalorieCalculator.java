...
public class ComplexCalorieCalculator implements CalorieCalculationFormula{
    ...
    public double calculate(Activity activity) {
        if (sex.equals("m")) {
            return (-55.0969 + (0.6309 * activity.getAverageHeartRate()) + (0.1966 * weight) + (0.2017 * age)) * (activity.getDuration() / 60.0) / 4.184;
        } else {
            return (-20.4022 + (0.4472 * activity.getAverageHeartRate()) + (0.1263 * weight) + (0.074 * age)) * (activity.getDuration() / 60.0) / 4.184;
        }
    }
}